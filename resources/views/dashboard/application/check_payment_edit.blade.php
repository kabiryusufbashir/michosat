    ...
    <div class="grid grid-cols-6">
        ...
        <div class="col-span-5 mt-2">
            ...
            <div class="bg-white py-3 px-6 ml-4 mr-8 text-gray-600 my-5 rounded">
                <h1 class="text-lg font-semibold py-4 w-full">Confirm {{ $check_payment->email }} Application fee Payment</h1>
                <div class="p-4">
                <!-- check_payment Edit  -->
                <form action="{{ route('check-payment-update', $check_payment->id) }}" method="POST" class="px-6 lg:px-8 py-8">
                    @csrf
                    @method('PATCH')
                    <div>
                        <label for="name" class="text-lg font-medium border-b-4 border-green-700">{{ $check_payment->year }} Application's Receipt</label><br>
                        <div class="grid grid-cols-2 gap-2 items-center mt-3">
                            <div class="mr-4">
                                <img class="w-full" src="{{ $check_payment->receipt }}" alt="{{ $check_payment->email }} Receipt">
                            </div>
                        </div>
                        @error('receipt')
                            {{$message}}
